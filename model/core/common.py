...
def pixel_shuffle(input, scale_factor, size=None):
    if size is None:
        batch_size, channels, in_height, in_width = input.size()
    else:
        batch_size, channels, in_height, in_width = size
...
class PixelShuffle(nn.Module):
    def __init__(self, scale_factor, size=None):
        super(PixelShuffle, self).__init__()
        self.scale_factor = scale_factor
        self.size = size
    
    def forward(self, x):
        return pixel_shuffle(x, self.scale_factor, self.size)
    def extra_repr(self):
